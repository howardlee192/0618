...
const client = createClient({
  ...
  // Requires a token with write access. Since this is local and the user is logged in via CLI,
  // wait, the CLI token isn't automatically available to a Node script unless we pass it.
  // We can just ask the user to create a token or we can use the CLI token.
})

// Hardcoded legacy projects metadata
const PROJECTS_DATA = [
  {
    id: "unsorted",
  ...
  },
  {
    ...
    folder: "yen", // Special folder name
    className: ""
  },
  {
    id: "endowing",
...
async function migrate() {
  ...
  for (const proj of PROJECTS_DATA) {
    ...
    // Map folder names matching public/projects structure
    ...
    // If cover was not explicitly found by name, just pick the first one
